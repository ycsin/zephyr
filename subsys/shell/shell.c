			 ...
			 bool use_colors, bool use_echo)
{
	...
	z_flag_echo_set(shell, IS_ENABLED(CONFIG_SHELL_ECHO_STATUS) && use_echo);
...
int shell_init(const struct shell *shell, const void *transport_config,
	       bool use_colors, bool use_echo, bool log_backend, uint32_t init_log_level)
{
	__ASSERT_NO_MSG(shell);
	...
	int err = instance_init(shell, transport_config, use_colors, use_echo);
