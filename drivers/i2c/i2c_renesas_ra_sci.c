...
#define DT_DRV_COMPAT renesas_ra_i2c_sci
...
#include "r_sci_i2c.h"
#ifdef CONFIG_I2C_RENESAS_RA_SCI_DTC
#include "r_dtc.h"
#endif

#include <soc.h>

LOG_MODULE_REGISTER(renesas_ra_i2c_sci, CONFIG_I2C_LOG_LEVEL);
...
struct sci_i2c_data {
	sci_i2c_instance_ctrl_t ctrl;
	i2c_master_cfg_t i2c_config;
	sci_i2c_extended_cfg_t ext_cfg;
...
static void calc_sci_iic_clock_setting(const struct device *dev, const uint32_t fsp_i2c_rate,
					 sci_i2c_clock_settings_t *clk_cfg);

/* FSP interruption handlers. */
extern void sci_i2c_txi_isr(void);
extern void sci_i2c_tei_isr(void);
extern void sci_i2c_rxi_isr(void);

static int renesas_ra_sci_i2c_configure(const struct device *dev, uint32_t dev_config)
{
	...
	calc_sci_iic_clock_setting(dev, data->i2c_config.rate, &data->ext_cfg.clock_settings);

	err = R_SCI_I2C_Close(&data->ctrl);
	if (err != FSP_SUCCESS) {
		LOG_ERR("Failed to configure I2C device");
		return -EIO;
	}

	err = R_SCI_I2C_Open(&data->ctrl, &data->i2c_config);
	if (err != FSP_SUCCESS) {
		LOG_ERR("Failed to configure I2C device");
		return -EIO;
	}
...
static int renesas_ra_sci_i2c_get_config(const struct device *dev, uint32_t *dev_config)
{
	struct sci_i2c_data *data = (struct sci_i2c_data *const)dev->data;
...
static int renesas_ra_sci_i2c_transfer(const struct device *dev, struct i2c_msg *msgs,
					 uint8_t num_msgs, uint16_t addr)
{
	...
	R_SCI_I2C_SlaveAddressSet(&data->ctrl, addr, addr_mode);
	...
	while (num_msgs > 0) {
		...
		if (current->flags & I2C_MSG_READ) {
			fsp_err = R_SCI_I2C_Read(&data->ctrl, current->buf, current->len,
						   next != NULL && (next->flags & I2C_MSG_RESTART));
		} else {
			fsp_err =
				R_SCI_I2C_Write(&data->ctrl, current->buf, current->len,
						  next != NULL && (next->flags & I2C_MSG_RESTART));
		}
...
static void renesas_ra_sci_i2c_async_done(const struct device *dev, struct sci_i2c_data *data,
					    int result)
{
...
static void renesas_ra_sci_i2c_async_iter(const struct device *dev)
{
	...
	if (data->msg_idx < (data->num_msgs - 1)) {
		...
		if (OPERATION(current) != OPERATION(next)) {
			if (!(next->flags & I2C_MSG_RESTART)) {
				...
				renesas_ra_sci_i2c_async_done(dev, data, -EIO);
				return;
			}
		}
	...
	if (current->flags & I2C_MSG_READ) {
		...
		fsp_err = R_SCI_I2C_Write(&data->ctrl, current->buf, current->len,
					    (next != NULL) && (next->flags & I2C_MSG_RESTART));
	}
	...
	if (fsp_err != FSP_SUCCESS) {
		...
		R_SCI_I2C_Abort(&data->ctrl);
		return;
	}
}

static int renesas_ra_sci_i2c_transfer_cb(const struct device *dev, struct i2c_msg *msgs,
					    ...
					    void *p_context)
{
	...
	renesas_ra_sci_i2c_async_iter(dev);
...
static void renesas_ra_sci_i2c_callback(i2c_master_callback_args_t *p_args)
{
	...
	if (data->cb != NULL) {
		...
		if (p_args->event == I2C_MASTER_EVENT_ABORTED) {
			...
			renesas_ra_sci_i2c_async_done(dev, data, 0);
		} else {
			data->msg_idx++;
			renesas_ra_sci_i2c_async_iter(dev);
		}
...
static int renesas_ra_sci_i2c_init(const struct device *dev)
{
	const struct sci_i2c_config *config = dev->config;
	struct sci_i2c_data *data = (struct sci_i2c_data *)dev->data;
	fsp_err_t fsp_err;
	int ret;
	...
	case I2C_MASTER_RATE_STANDARD:
		calc_sci_iic_clock_setting(dev, data->i2c_config.rate,
	...
	fsp_err = R_SCI_I2C_Open(&data->ctrl, &data->i2c_config);
					 ...
					 sci_i2c_clock_settings_t *clk_cfg)
{
	...
	if (use_mddr) {
		for (uint32_t temp_brr = brr; temp_brr > 0; temp_brr--) {
			...
			calculated_bitrate = (uint32_t)(peripheral_clock /
							(divisor_bitrate_multiple *
							 (MDDR_SCALE_FACTOR / ((double)temp_mddr)) *
							 (temp_brr + 1)));
...
#ifdef CONFIG_I2C_CALLBACK
	.transfer_cb = renesas_ra_sci_i2c_transfer_cb,
#endif /* CONFIG_I2C_CALLBACK */
};
...
#ifndef CONFIG_I2C_RENESAS_RA_SCI_DTC
#define SCI_I2C_DTC_INIT(index)
...
#define SCI_I2C_DTC_INIT(index)                                                                    \
	...
	.rx_transfer_cfg =                                                                         \
		{                                                                                  \
			.p_info = &sci_i2c_data_##index.rx_transfer_info,                          \
			.p_extend = &sci_i2c_data_##index.rx_transfer_cfg_extend,                  \
	},                                                                                         \
	.rx_transfer =                                                                             \
		{                                                                                  \
			.p_ctrl = &sci_i2c_data_##index.rx_transfer_ctrl,                          \
			.p_cfg = &sci_i2c_data_##index.rx_transfer_cfg,                            \
	...
	.tx_transfer_cfg =                                                                         \
		{                                                                                  \
			.p_info = &sci_i2c_data_##index.tx_transfer_info,                          \
			.p_extend = &sci_i2c_data_##index.tx_transfer_cfg_extend,                  \
	},                                                                                         \
	.tx_transfer = {                                                                           \
		.p_ctrl = &sci_i2c_data_##index.tx_transfer_ctrl,                                  \
		.p_cfg = &sci_i2c_data_##index.tx_transfer_cfg,                                    \
		.p_api = &g_transfer_on_dtc,                                                       \
	},
...
#define RXI_TRANSFER(index)                                                                        \
	...
	INTC2_DT_CONNECT_INLINE_BY_NAME(DT_INST_PARENT(index), rxi,                               \
		    DT_IRQ_BY_NAME(DT_INST_PARENT(index), rxi, priority), sci_i2c_rxi_isr,         \
		    DEVICE_DT_INST_GET(index), 0);                                                 \
	intc2_enable((struct intc2_spec)INTC2_DT_SPEC_GET_BY_NAME(DT_INST_PARENT(index), rxi));
#endif

#define SCI_I2C_RA_INIT(index)                                                                     \
	static void renesas_ra_sci_i2c_irq_config_func##index(const struct device *dev)            \
	{                                                                                          \
		RXI_TRANSFER(index)                                                                \
                                                                                                   \
		/* txi */                                                                          \
		...
		INTC2_DT_CONNECT_INLINE_BY_NAME(DT_INST_PARENT(index), txi,                       \
			    ...
			    sci_i2c_txi_isr, DEVICE_DT_INST_GET(index), 0);                        \
	...
	static struct sci_i2c_data sci_i2c_data_##index = {                                        \
		.i2c_config =                                                                      \
			{                                                                          \
				...
				.p_callback = renesas_ra_sci_i2c_callback,                         \
		...
		.ext_cfg =                                                                         \
			{                                                                          \
				...
				.clock_settings.bitrate_modulation = DT_INST_NODE_HAS_PROP(        \
					index, bit_rate_modulation),                               \
			},                                                                         \
		SCI_I2C_DTC_INIT(index)};                                                          \
	I2C_DEVICE_DT_INST_DEFINE(index, renesas_ra_sci_i2c_init, NULL, &sci_i2c_data_##index,     \
				  &sci_i2c_config_##index, POST_KERNEL,                            \
				  CONFIG_I2C_INIT_PRIORITY, &renesas_ra_sci_i2c_driver_api);

DT_INST_FOREACH_STATUS_OKAY(SCI_I2C_RA_INIT)
