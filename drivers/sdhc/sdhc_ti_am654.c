...
struct ti_am654_ss_regs {
	uint8_t RESERVED_1[0x14];     /**< Reserved, offset: 0x00-0x14 */
	volatile uint32_t CTL_CFG_2;  /**< Controller Config 2, offset: 0x14 */
	uint8_t RESERVED_4[0xE8];     /**< Reserved, offset: 0x18-0x100 */
	volatile uint32_t PHY_CTRL_1; /**< PHY Control 1, offset: 0x100 */
	uint8_t RESERVED_2[0x08];     /**< Reserved, offset: 0x104-0x10C */
	volatile uint32_t PHY_CTRL_4; /**< PHY Control 4, offset: 0x10C */
	volatile uint32_t PHY_CTRL_5; /**< PHY Control 5, offset: 0x110 */
	uint8_t RESERVED_3[0x1C];     /**< Reserved, offset: 0x114-0x130 */
	volatile uint32_t PHY_STAT_1; /**< PHY Status 1, offset: 0x130 */
};

#ifdef CONFIG_SD_CMD_TIMEOUT
#define AM654_CMD_TIMEOUT  CONFIG_SD_CMD_TIMEOUT
#else
#define AM654_CMD_TIMEOUT (200)
#endif /* CONFIG_SD_CMD_TIMEOUT */

#ifdef CONFIG_SD_DATA_TIMEOUT
#define AM654_DATA_TIMEOUT  CONFIG_SD_DATA_TIMEOUT
#else
#define AM654_DATA_TIMEOUT (10000)
#endif /* CONFIG_SD_DATA_TIMEOUT */
...
struct ti_am654_hc_regs {
	volatile uint32_t SYS_ADDR;            /**< SDMA System Address, offset: 0x00 */
	volatile uint16_t BLOCK_SIZE;          /**< Block Size, offset: 0x04 */
	volatile uint16_t BLOCK_COUNT;         /**< Block Count, offset: 0x06 */
	volatile uint32_t ARGUMENT1;           /**< Argument1, offset: 0x08 */
	volatile uint16_t TRANSFER_MODE;       /**< Transfer Mode, offset: 0x0C */
	volatile uint16_t COMMAND;             /**< Command, offset: 0x0E */
	volatile uint32_t RESPONSE[4];         /**< Response Register 0-3, offset: 0x10-0x1C */
	volatile uint32_t DATA_PORT;           /**< Buffer Data Port, offset: 0x20 */
	volatile uint32_t PRESENTSTATE;        /**< Present State, offset: 0x24 */
	volatile uint8_t HOST_CONTROL1;        /**< Host Control 1, offset: 0x28 */
	volatile uint8_t POWER_CONTROL;        /**< Power Control, offset: 0x29 */
	volatile uint8_t BLOCK_GAP_CONTROL;    /**< Block Gap Control, offset: 0x2A */
	volatile uint8_t WAKEUP_CONTROL;       /**< Wakeup Control, offset: 0x2B */
	volatile uint16_t CLOCK_CONTROL;       /**< Clock Control, offset: 0x2C */
	volatile uint8_t TIMEOUT_CONTROL;      /**< Timeout Control, offset: 0x2E */
	volatile uint8_t SOFTWARE_RESET;       /**< Software Reset, offset: 0x2F */
	volatile uint16_t NORMAL_INTR_STS;     /**< Normal IRQ Status, offset: 0x30 */
	volatile uint16_t ERROR_INTR_STS;      /**< Error IRQ Status, offset: 0x32 */
	volatile uint16_t NORMAL_INTR_STS_ENA; /**< Normal IRQ Status Enable, offset: 0x34 */
	volatile uint16_t ERROR_INTR_STS_ENA;  /**< Error IRQ Status Enable, offset: 0x36 */
	volatile uint16_t NORMAL_INTR_SIG_ENA; /**< Normal IRQ Signal Enable, offset: 0x38 */
	volatile uint16_t ERROR_INTR_SIG_ENA;  /**< Error IRQ Signal Enable, offset: 0x3A */
	volatile uint16_t AUTOCMD_ERR_STS;     /**< Auto CMD Error Status, offset: 0x3C */
	volatile uint16_t HOST_CONTROL2;       /**< Host Control 2, offset: 0x3E */
	volatile uint64_t CAPABILITIES;        /**< Capabilities, offset: 0x40 */
	volatile uint64_t MAX_CURRENT_CAP;     /**< Maximum Current Capabilities, offset: 0x48 */
	uint8_t RESERVED_1[0x8];               /**< Reserved, offset: 0x50-0x58 */
	volatile uint64_t ADMA_SYS_ADDRESS;    /**< ADMA System Address, offset: 0x58 */
	uint8_t RESERVED_2[0x20];              /**< Reserved, offset: 0x60-0x80 */
};
...
#if defined(CONFIG_DCACHE) && defined(CONFIG_DCACHE_LINE_SIZE)
#define SDHC_CACHE_ALIGNMENT CONFIG_DCACHE_LINE_SIZE
#else
#define SDHC_CACHE_ALIGNMENT (128) /* safe default */
#endif
...
/* ADMA specific fields */
	...
	/* ADMA descriptors */
	struct ti_am654_adma2_descriptor descs[CONFIG_SDHC_TI_AM654_ADMA_DESC_LEN];
	/* Cache-aligned residual buffers for DMA */
	uint8_t residual_start[SDHC_CACHE_ALIGNMENT] __aligned(SDHC_CACHE_ALIGNMENT);
	uint8_t residual_end[SDHC_CACHE_ALIGNMENT] __aligned(SDHC_CACHE_ALIGNMENT);
...
static int ti_am654_reset(const struct device *dev, enum ti_am654_reset_type type)
{
	...
	while (hc_regs->SOFTWARE_RESET & type) {
		...
		if (retries-- == 0) {
			LOG_ERR("SDHC software reset timed out");
			return -ETIMEDOUT;
		}
...
static void ti_am654_read_cmd_resp(const struct device *dev, struct sdhc_command *cmd)
{
	...
	uint32_t r0 = hc_regs->RESPONSE[0];
	uint32_t r1 = hc_regs->RESPONSE[1];
	uint32_t r2 = hc_regs->RESPONSE[2];
	uint32_t r3 = hc_regs->RESPONSE[3];
	...
	case SD_RSP_TYPE_R2:
		...
		cmd->response[3] = ((r3 & GENMASK(23, 0)) << 8) | (r2 >> 24);
...
#ifdef CONFIG_SDHC_TI_AM654_ENABLE_ADMA
static size_t ti_am654_get_dcache_line_size(void)
{
...
static void ti_am654_writeback_residuals(const struct device *dev, struct sdhc_data *dat)
{
	...
	size_t cache_line_size = ti_am654_get_dcache_line_size();
					    ...
					    bool is_write)
{
	...
	if (ARRAY_SIZE(data->descs) * TI_AM654_ADMA2_DESC_LENGTH_MAX < length_left) {
		LOG_ERR("number of descriptors %zu is less than required", ARRAY_SIZE(data->descs));
		return -EINVAL;
	}
...
start_adma:
	if (IS_ENABLED(CONFIG_CACHE_MANAGEMENT) && IS_ENABLED(CONFIG_DCACHE)) {
		...
		if (is_write) {
			...
			size_t aligned_len = (dat->blocks * dat->block_size) -
					     start_residual_bytes - end_residual_bytes;

			/* invd the aligned region, the residual buffers are already invalidated */
				       ...
				       bool is_write)
{
	...
	if (dat->blocks > 1) {
		...
		if (IS_ENABLED(CONFIG_SDHC_TI_AM654_ENABLE_AUTO_STOP) &&
		    IS_ENABLED(CONFIG_SDHC_TI_AM654_ENABLE_ADMA) &&
		    DEV_DATA(dev)->ios.timing == SDHC_TIMING_SDR104) {
			    ...
			    struct sdhc_data *dat)
{
	...
	k_event_clear(&data->irq_event, GENMASK(31, 0));
	ti_am654_init_cmd_cfg(&cfg, cmd, dat);

	rv = WAIT_FOR(((hc_regs->PRESENTSTATE & TI_AM654_PRESENTSTATE_INHIBIT_CMD) == 0),
		      cmd->timeout_ms * 1000, NULL);
	if (rv == false) {
		LOG_ERR("timed out waiting for command line to be idle");
		return -ETIMEDOUT;
	}
	...
	if (cfg.data_present) {
		...
		rv = WAIT_FOR(((hc_regs->PRESENTSTATE & TI_AM654_PRESENTSTATE_INHIBIT_DAT) == 0),
			      dat->timeout_ms * 1000, NULL);
		if (rv == false) {
			LOG_ERR("timed out waiting for data line to be idle");
			return -ETIMEDOUT;
		}
	}
...
static int ti_am654_send_tuning_data(const struct device *dev)
{
	...
	cmd.timeout_ms = AM654_CMD_TIMEOUT;
	cmd.retries = 0;
	cmd.arg = 0;
	...
	dat.data = rd_buf;
	dat.timeout_ms = AM654_DATA_TIMEOUT;
	dat.blocks = 1;
...
static int ti_am654_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_SDHC_TI_AM654_ENABLE_ADMA)) {
		...
		hc_regs->HOST_CONTROL2 |= (TI_AM654_HOST_CONTROL2_ADMA2_LEN_MODE |
					   TI_AM654_HOST_CONTROL2_BIT64_ADDRESSING);
	}
...
static void ti_am654_isr(const struct device *dev)
{
	...
	if (nstatus & TI_AM654_NORMAL_INTR_CARD_INSERTION) {
		...
		if (data->callback != NULL && ti_am654_get_card_present(dev)) {
...
#define TI_AM654_INIT(n)                                                                           \
	...
	{                                                                                          \
		INTC2_DT_INST_CONNECT_INLINE(n, DT_INST_IRQ(n, priority), ti_am654_isr,               \
