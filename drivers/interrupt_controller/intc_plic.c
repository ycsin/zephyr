...
static void plic_irq_handler(const struct device *dev)
{
	...
	if (stat.irq_count[irq_total] < IRQ_COUNT_MAX) {
...
static int cmd_stats_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	for (int cpu_id = 0; cpu_id < arch_num_cpus(); cpu_id++) {
		shell_fprintf(sh, SHELL_NORMAL, "  %*sCPU%2d", CONFIG_PLIC_IRQ_COUNT_WIDTH - 5, "",
			      cpu_id);
	}
	if (CONFIG_MP_NUM_CPUS > 1) {
		shell_fprintf(sh, SHELL_NORMAL, "  %*sTotal", CONFIG_PLIC_IRQ_COUNT_WIDTH - 5, "");
	}
	...
	for (int i = 0; i < config->nr_irqs; i++) {
		...
		plic_irq_count_t total_hits = stat.irq_count[offset + i];
		...
		for (int cpu_id = 0; cpu_id < arch_num_cpus(); cpu_id++) {
			...
			shell_fprintf(sh, SHELL_NORMAL, "  " IRQ_COUNT_FMT,
				      CONFIG_PLIC_IRQ_COUNT_WIDTH, stat.irq_count[irq_index]);
		}
		if (CONFIG_MP_NUM_CPUS > 1) {
			shell_fprintf(sh, SHELL_NORMAL, "  " IRQ_COUNT_FMT,
				      CONFIG_PLIC_IRQ_COUNT_WIDTH,
...
#define PLIC_INTC_IRQ_COUNT_BUF_DEFINE(n)                                                          \
	static plic_irq_count_t local_irq_count_##n[COND_CODE_1(                                   \
		CONFIG_MP_MAX_NUM_CPUS, (1), (UTIL_INC(CONFIG_MP_MAX_NUM_CPUS)))]                  \
						   [PLIC_MIN_IRQ_NUM(n)];
