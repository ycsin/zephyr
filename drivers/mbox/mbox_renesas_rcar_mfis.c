...
typedef void (*rcar_mfis_config_irq_t)(void);
			  ...
			  const struct mbox_msg *msg)
{
	...
	/* Check if remote side has consumed the previous data */
	eicr_val = rcar_mfis_read(dev, eicr);
	if (eicr_val & RCAR_MFIS_IRQ_BIT) {
		/* Interrupt not cleared by remote side yet */
		return -EBUSY;
	}

	/* Signalling mode */
	if (msg == NULL) {
		/* Issue interrupt with default zero'd payload */
		key = k_spin_lock(&data->wr_lock);
		rcar_mfis_write_prot(dev, channel, embr, 0);
...
#define RCAR_MFIS_INSTANCE_DEFINE(n)								\
	BUILD_ASSERT(DT_NUM_IRQS(DT_DRV_INST(n)) == RCAR_MFIS_MAX_CHANNELS,			\
		     "The interrupt list must contain one entry per supported channel");	\
												\
	LISTIFY(DT_NUM_IRQS(DT_DRV_INST(n)), RCAR_MFIS_ISR_DEFINE, (), n)			\
												\
	static void rcar_mfis_irq_config_##n(void)						\
	{											\
