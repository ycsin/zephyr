...
struct i2s_mcux_q_entry {
	void *mem_block;
	size_t size;
};

struct i2s_mcux_data {
	struct i2s_mcux_stream rx;
	struct i2s_mcux_q_entry rx_in_msgs[CONFIG_I2S_MCUX_FLEXCOMM_RX_BLOCK_COUNT];
	void *rx_out_msgs[CONFIG_I2S_MCUX_FLEXCOMM_RX_BLOCK_COUNT];
	struct dma_block_config rx_dma_blocks[CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS];

	struct i2s_mcux_stream tx;
					 ...
					 const struct i2s_config *i2s_cfg, i2s_config_t *fsl_cfg)
{
	...
	case I2S_OPT_BIT_CLK_CONTROLLER | I2S_OPT_FRAME_CLK_TARGET:
		...
		break;
	default:
		LOG_ERR("Unsupported master/slave configuration");
		return -EINVAL;
	}
...
static inline void i2s_purge_stream_buffers(struct i2s_mcux_stream *stream,
...
static void i2s_mcux_tx_stream_disable(const struct device *dev, bool drop)
{
	const struct i2s_mcux_config *cfg = dev->config;
	struct i2s_mcux_data *dev_data = dev->data;
	struct i2s_mcux_stream *stream = &dev_data->tx;
...
static void i2s_mcux_rx_stream_disable(const struct device *dev, bool drop)
{
	const struct i2s_mcux_config *cfg = dev->config;
	struct i2s_mcux_data *dev_data = dev->data;
	struct i2s_mcux_stream *stream = &dev_data->rx;
	I2S_Type *base = cfg->base;
				       ...
				       struct i2s_mcux_q_entry *queue_entry)
{
	...
	struct dma_block_config *dma_blocks;
	struct i2s_mcux_stream *stream;
	uint32_t fifo_addr;
	int num_blocks;
	...
	if (dir == I2S_DIR_RX) {
		stream = &dev_data->rx;
		dma_blocks = dev_data->rx_dma_blocks;
		num_blocks = CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS;
		fifo_addr = (uint32_t)&base->FIFORD;
		memset(dma_blocks, 0, sizeof(dev_data->rx_dma_blocks));
	} else {
		stream = &dev_data->tx;
		dma_blocks = dev_data->tx_dma_blocks;
		num_blocks = CONFIG_I2S_MCUX_FLEXCOMM_DMA_TX_BLOCKS;
		fifo_addr = (uint32_t)&base->FIFOWR;
		memset(dma_blocks, 0, sizeof(dev_data->tx_dma_blocks));
	}

	stream->dma_cfg.head_block = &dma_blocks[0];

	for (int i = 0; i < num_blocks; i++) {
		blk_cfg = &dma_blocks[i];

		if (dir == I2S_DIR_RX) {
			blk_cfg->source_address = fifo_addr;
			blk_cfg->source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
			blk_cfg->dest_address = (uint32_t)((queue_entry + i)->mem_block);
		} else {
			blk_cfg->dest_address = fifo_addr;
			blk_cfg->dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
			blk_cfg->source_address = (uint32_t)((queue_entry + i)->mem_block);
		}

		blk_cfg->block_size = (uint32_t)((queue_entry + i)->size);

		if (i < num_blocks - 1) {
			blk_cfg->next_block = &dma_blocks[i + 1];
			if (dir == I2S_DIR_RX) {
...
static int i2s_mcux_tx_stream_start(const struct device *dev)
{
	...
	for (int i = 0; i < CONFIG_I2S_MCUX_FLEXCOMM_DMA_TX_BLOCKS; i++) {
...
{
	int ret = 0;
	struct i2s_mcux_q_entry queue_entry[CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS];
	const struct i2s_mcux_config *cfg = dev->config;
	struct i2s_mcux_data *dev_data = dev->data;
	struct i2s_mcux_stream *stream = &dev_data->rx;
	I2S_Type *base = cfg->base;
	...
	/*
	 * Need at least as many free buffers on the RX memory slab as the
	 * number of configured DMA blocks for reliable DMA reception.
	 */
	if (num_of_bufs < CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS) {
		return -EINVAL;
	}

	for (int i = 0; i < CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS; i++) {
	...
	i2s_mcux_config_dma_blocks(dev, I2S_DIR_RX, queue_entry);
...
static int i2s_mcux_trigger_start(const struct device *dev, enum i2s_dir dir,
				  struct i2s_mcux_stream *stream)
{
	int ret;
...
static int i2s_mcux_trigger(const struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd)
{
	...
	case I2S_TRIGGER_START:
		ret = i2s_mcux_trigger_start(dev, dir, stream);
		break;
...
#define I2S_DMA_CHANNELS(id)                                                                       \
	.tx = {.dev_dma = UTIL_AND(DT_INST_DMAS_HAS_NAME(id, tx),                                  \
	       ...
	       .dma_cfg =                                                                          \
		       {                                                                           \
			       ...
			       .block_count = CONFIG_I2S_MCUX_FLEXCOMM_DMA_TX_BLOCKS,              \
		       }},                                                                         \
	.rx = {.dev_dma = UTIL_AND(DT_INST_DMAS_HAS_NAME(id, rx),                                  \
	       ...
	       .dma_cfg = {                                                                        \
		       ...
		       .complete_callback_en = true,                                               \
		       .block_count = CONFIG_I2S_MCUX_FLEXCOMM_DMA_RX_BLOCKS,                      \
	       }}
...
#define I2S_MCUX_FLEXCOMM_DEVICE(id)                                                               \
	...
	static void i2s_mcux_config_func_##id(const struct device *dev)                            \
	{                                                                                          \
		INTC2_DT_INST_CONNECT_INLINE(id, DT_INST_IRQ(id, priority), i2s_mcux_isr,             \
