...
static int init_kbc0(const struct device *dev)
{
	...
	p92_hw->ACTV = MCHP_PORT92_ACTV_ENABLE;
				    ...
				    enum espi_virtual_peripheral periph)
{
	...
	if (regs->EC_STS & MCHP_ACPI_EC_STS_IBF) {
		...
		pvt_evt->type = ESPI_EVENT_DATA_ACPI_TYPE_HOST_TO_EC_DATA;
		if ((regs->EC_STS & MCHP_ACPI_EC_STS_CMD) != 0) {
			pvt_evt->type = ESPI_EVENT_DATA_ACPI_TYPE_HOST_TO_EC_CMD;
		}
				    ...
				    enum espi_virtual_peripheral periph)
{
	...
	evt.evt_details = periph;
	pvt_evt->type = ESPI_EVENT_DATA_ACPI_TYPE_HOST_RD_EC_TO_HOST;
	pvt_evt->data = 0;
