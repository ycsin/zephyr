...
#define DPU_FB_PITCH_ALIGN (32U)

static const uint32_t dpu_supported_fmts =
...
struct dpu_data {
	...
	uint32_t fb_bytes;
	/* Lazy-start display on first write */
	bool started;
};
		     ...
		     const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	const uint8_t *src;
	uint8_t *dst;
	bool buf_aligned = ((uintptr_t)buf % 32U) == 0U;
	uint32_t src_stride_bytes = (uint32_t)desc->pitch * data->pixel_bytes;

	__ASSERT(((uint32_t)desc->height * src_stride_bytes) <= desc->buf_size,
		 "Input buffer too small");

	LOG_DBG("W=%d, H=%d @%d,%d", desc->width, desc->height, x, y);

	if ((x == 0) && (y == 0) && (desc->width == config->display_timing.width) &&
	    (desc->height == config->display_timing.height) &&
	    (src_stride_bytes == data->stride_bytes) && buf_aligned) {
				...
				const enum display_pixel_format pixel_format)
{
	...
	if (!(pixel_format & dpu_supported_fmts)) {
	...
	data->stride_bytes = ROUND_UP((uint32_t)config->display_timing.width * data->pixel_bytes,
				      DPU_FB_PITCH_ALIGN);
	data->fb_bytes = data->stride_bytes * config->display_timing.height;
...
static void dpu_isr(const struct device *dev)
{
	...
	if ((status & kDPU_Group0ExtDst0ShadowLoadInterrupt) != 0U) {
		k_sem_give(&data->sem);
	}
}
...
static int dpu_init(const struct device *dev)
{
	...
	memset(config->fb_ptr, 0, data->fb_bytes * CONFIG_DPU_FB_NUM);
	...
	/* Trigger shadow load for content stream. */
	dpu_trigger_content_stream_shadowload(dev);

	DPU_EnableInterrupts(APP_DPU_DC, 0, kDPU_Group0ExtDst0ShadowLoadInterrupt);

	/* Trigger shadow load for domain blend. */
...
#define DPU_FRAMEBUFFER_DECL(id)                                                                   \
	...
	dpu_frame_buffer_##id[CONFIG_DPU_FB_NUM * DT_INST_PROP(id, height) *                       \
			      ROUND_UP((DT_INST_PROP(id, width) * 4U), DPU_FB_PITCH_ALIGN)]
