...
/* Hardware DMA descriptor - layout must match USBC peripheral */
...
struct udc_sam_usbc_data {
	...
	uint8_t _pad[3];        /* MUST keep struct 4-byte aligned without holes*/
...
static void sam_usbc_isr_sta_dbg(Usbc *base, uint8_t ep_idx, uint32_t sr)
{
...
static void sam_usbc_ep_isr_sta(Usbc *base, uint8_t ep_idx)
{
...
static ALWAYS_INLINE void sam_usbc_freeze_clk(Usbc *const base)
{
	base->USBCON |= USBC_USBCON_FRZCLK;
}

static ALWAYS_INLINE void sam_usbc_unfreeze_clk(Usbc *const base)
{
	base->USBCON &= ~USBC_USBCON_FRZCLK;

	/* Clears within a few clock cycles per datasheet, no timeout needed */
...
sam_usbc_desc_table *sam_usbc_get_desc(const struct device *const dev,
...
static inline uint8_t virt_addr_to_idx(const uint8_t virt_addr)
{
	...
	__ASSERT_NO_MSG(ep_num < NUM_OF_EP_MAX);
...
static inline uint8_t virt_to_phys_ep(const struct device *const dev, const uint8_t virt_addr)
{
...
static int alloc_phys_ep(const struct device *const dev, const uint8_t virt_addr)
{
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static void free_phys_ep(const struct device *const dev, const uint8_t virt_addr)
{
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
	uint8_t idx = virt_addr_to_idx(virt_addr);
...
static uint8_t phys_ep_to_virt_addr(const struct device *const dev, const uint8_t phys_ep)
{
	const struct udc_sam_usbc_config *config = dev->config;
...
static void sam_usbc_phys_ep_reset(const struct device *const dev)
{
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static int sam_usbc_prep_out(const struct device *const dev,
...
static int sam_usbc_prep_in(const struct device *const dev,
			    struct net_buf *const buf,
			    struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
...
static int sam_usbc_handle_evt_dout(const struct device *const dev,
				    struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static ALWAYS_INLINE void sam_usbc_thread_handler(const struct device *const dev)
{
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static void sam_usbc_handle_setup_isr(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
	Usbc *const base = config->base;
...
static void sam_usbc_handle_out_isr(const struct device *const dev, const uint8_t phys_ep)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
	Usbc *const base = config->base;
...
static void sam_usbc_handle_in_isr(const struct device *const dev, const uint8_t phys_ep)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
	Usbc *const base = config->base;
...
static void sam_usbc_handle_ep_isr(const struct device *const dev, const uint8_t ep_idx)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
	uint32_t sr = base->UESTA[ep_idx];
...
/* Main ISR handler */
static void sam_usbc_isr_handler(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
	...
	if ((sr & USBC_UDINT_EORST) && (inte & USBC_UDINTE_EORSTE)) {
		 ...
		 * Reset software EP tracking to match, then reconfigure EP0
		 * immediately before host sends SETUP.
		 */
		sam_usbc_phys_ep_reset(dev);
		priv->phys_ep_used |= BIT(0);
		priv->virt_to_phys[0] = 0;
		priv->virt_to_phys[NUM_OF_EP_MAX] = 0;
	...
	if ((sr & USBC_UDINT_SUSP) && (inte & USBC_UDINTE_SUSPE)) {
		...
		/* Clears within a few clock cycles per datasheet */
...
static int udc_sam_usbc_ep_enqueue(const struct device *const dev,
				   struct udc_ep_config *const ep_cfg,
				   struct net_buf *buf)
{
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static int udc_sam_usbc_ep_dequeue(const struct device *const dev,
				   struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_ep_enable(const struct device *const dev,
				  struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static int udc_sam_usbc_ep_disable(const struct device *const dev,
				   struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_ep_set_halt(const struct device *const dev,
				    struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_ep_clear_halt(const struct device *const dev,
				      struct udc_ep_config *const ep_cfg)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
...
static int udc_sam_usbc_set_address(const struct device *const dev, const uint8_t addr)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_host_wakeup(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static enum udc_bus_speed udc_sam_usbc_device_speed(const struct device *const dev)
{
...
static int udc_sam_usbc_enable(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_disable(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	Usbc *const base = config->base;
...
static int udc_sam_usbc_init(const struct device *const dev)
{
...
static int udc_sam_usbc_shutdown(const struct device *const dev)
{
...
/*
 * UDC API: Lock
 * k_sched_lock prevents preemption while holding the mutex to ensure
 * timely USB register access, same pattern as udc_dwc2/udc_sam0.
 */
static void udc_sam_usbc_lock(const struct device *const dev)
{
	k_sched_lock();
	udc_lock_internal(dev, K_FOREVER);
}
...
/* UDC API: Unlock */
static void udc_sam_usbc_unlock(const struct device *const dev)
{
	udc_unlock_internal(dev);
	k_sched_unlock();
}
...
static int udc_sam_usbc_driver_preinit(const struct device *const dev)
{
	const struct udc_sam_usbc_config *config = dev->config;
	struct udc_sam_usbc_data *priv = udc_get_private(dev);
	...
	{									      \
		while (true) {							      \
			sam_usbc_thread_handler(dev);				      \
										      ...
										      \
	static void udc_sam_usbc_make_thread_##n(const struct device *const dev)      \
	{									      \
		struct udc_sam_usbc_data *priv = udc_get_private(dev);		      \
										      \
