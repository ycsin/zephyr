...
static void mcux_tsi_process_channel(const struct device *dev, uint8_t channel_idx)
{
	...
	if (!ch->active && ch->delta > (int16_t)config->touch_threshold) {
		ch->active = true;
	} else if (ch->active && ch->delta < (int16_t)config->release_threshold) {
		ch->active = false;
	}
...
static void mcux_tsi_process_work_handler(struct k_work *work)
{
...
static int mcux_tsi_init(const struct device *dev)
{
	...
	k_work_init(&data->process_work, mcux_tsi_process_work_handler);
	...
	/* Calibrate channels */
	LOG_DBG("Calibrating TSI channels...");
